		...
		t.Run(test.name, func(t *testing.T) {
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "certs.expires_in_seconds":
					...
					attrVal, ok := dp.Attributes().Get("filename")
					assert.True(t, ok)
					assert.EqualValues(t, "filename-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("host.name")
					assert.True(t, ok)
					assert.EqualValues(t, "host.name-val", attrVal.Str())
					attrVal, ok = dp.Attributes().Get("cn")
					assert.True(t, ok)
					assert.EqualValues(t, "cn-val", attrVal.Str())
				}
			}
		})
	}
